...
public:
	...
	void init() __attribute__((always_inline))
	{
		...
		//C. TMR1_CSCTRL1 Timer Channel Comparator Status and Control Register       ----------------------------------------------------------------------
			...
			IMXRT_TMR1.CH[_TMR1_CONTROLLER_CH_].CSCTRL |= TMR_CSCTRL_ROC; //Set Bit
		...
		//I. Timer Channel Enable Register (TMR1_ENBL)                                -----------------------------------------------
			...
			IMXRT_TMR1.ENBL |= (1 << _TMR1_CONTROLLER_CH_);
	...
	\b Reference XBAR and IOMUX input selections → IOMUXC_GPR_GPR6 Manual Page 347. */
	void init_pins() __attribute__((always_inline))
	{
		#if _TMR1_CONTROLLER_CH_ == 0
			IOMUXC_SW_MUX_CTL_PAD_GPIO_B0_00 = 1; //Assign IOPAD for CH0
			IOMUXC_GPR_GPR6 |= IOMUXC_GPR_GPR6_QTIMER1_TRM1_INPUT_SEL; //Set CH1 input to XBAR
		#elif _TMR1_CONTROLLER_CH_ == 1
			IOMUXC_SW_MUX_CTL_PAD_GPIO_B0_01 = 1; //Assign IOPAD for CH1
			IOMUXC_GPR_GPR6 |= IOMUXC_GPR_GPR6_QTIMER1_TRM2_INPUT_SEL; //Set CH2 input to XBAR
		#elif _TMR1_CONTROLLER_CH_ == 2
			IOMUXC_SW_MUX_CTL_PAD_GPIO_B0_02 = 1; //Assign IOPAD to CH2
			IOMUXC_GPR_GPR6 |= IOMUXC_GPR_GPR6_QTIMER1_TRM3_INPUT_SEL; //Set CH3 input to XBAR
		#elif _TMR1_CONTROLLER_CH_ == 3
			static_assert(false, "This Channel has no IO Pin");
		#endif
	 ...
	 * \attention This function does not configure the pin and assign it to the QTMR module, for that TMR1Controller::init_pin() must be called. It can only be used if the input is actually routed to GPIO instead of XBAR. */
	...
	//14
	/** @brief Reset Counter and output flags. \b Reference Manual Page 332. 
	...
	{
		IOMUXC_GPR_GPR2 |= IOMUXC_GPR_GPR2_QTIMER1_TMR_CNTS_FREEZE; //Set Bit
	}


	//15
	/** @brief Enable counting normally. \b Reference Manual Page 332.
	 * \attention This setting applies to all the channels of the Timer 1 group. */
	void static timers_anti_freeze() __attribute__((always_inline))
	{
		IOMUXC_GPR_GPR2 &= ~IOMUXC_GPR_GPR2_QTIMER1_TMR_CNTS_FREEZE; //Unset Bit
	}
